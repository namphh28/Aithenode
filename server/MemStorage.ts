...
export class MemStorage implements IStorage {
  ...
  private async initializeData(): Promise<void> {
    ...
    const categories = [
      ...
      educatorCount: cat.educatorCount || null
    ...
    })) satisfies InsertEducatorProfile[];
    ...
    const testimonials = [
      ...
      isVisible: testimonial.isVisible || null
    })) satisfies InsertTestimonial[];
    ...
    for (const subjectName of mathSubjects) {
      const subject = await this.createSubject({ 
        categoryId: 1, 
        name: subjectName, 
        description: `Learn ${subjectName} with expert tutors` || null 
      } satisfies InsertSubject);
    ...
    for (const subjectName of programmingSubjects) {
      const subject = await this.createSubject({ 
        categoryId: 2, 
        name: subjectName, 
        description: `Master ${subjectName} with practical projects` || null 
      } satisfies InsertSubject);
    ...
    for (const subjectName of languageSubjects) {
      const subject = await this.createSubject({ 
        categoryId: 3, 
        name: subjectName, 
        description: `Become fluent in ${subjectName}` || null 
      } satisfies InsertSubject);
    ...
    // Create some reviews
    const reviews = [
      { educatorId: 1, studentId: 4, rating: 5, comment: "Dr. Johnson explained complex calculus concepts in a way that finally made sense to me.", sessionId: null },
      { educatorId: 2, studentId: 4, rating: 5, comment: "James is an excellent programming tutor. I learned React in just a few weeks!", sessionId: null },
      { educatorId: 3, studentId: 5, rating: 5, comment: "My Spanish improved dramatically after just a month of lessons with Maria.", sessionId: null }
    ].map(review => ({
      educatorId: review.educatorId,
  ...
  async createForumReply(replyData: InsertForumReply): Promise<ForumReply> {
    ...
    // Update reply count on parent post or reply
    if (reply.parentReplyId) {
      const parentReply = this.forumReplies.get(reply.parentReplyId);
      if (parentReply) {
        parentReply.nestedReplies++;
        this.forumReplies.set(reply.parentReplyId, parentReply);
      }
    } else {
  ...
  async getForumReplies(postId: number): Promise<ForumReply[]> {
    return Array.from(this.forumReplies.values())
      .filter(reply => reply.postId === postId && !reply.parentReplyId)
  ...
  async likeForumReply(replyId: number, userId: number): Promise<ForumReply | undefined> {
    ...
    const updatedReply: ForumReply = {
      ...reply,
      likes: reply.likes + 1,
      dislikes: dislikedBy.includes(userId) ? reply.dislikes - 1 : reply.dislikes,
      likedBy: newLikedBy,
  ...
  async dislikeForumReply(replyId: number, userId: number): Promise<ForumReply | undefined> {
    ...
    const updatedReply: ForumReply = {
      ...
      likes: likedBy.includes(userId) ? reply.likes - 1 : reply.likes,
      dislikedBy: newDislikedBy,
      likedBy: newLikedBy
    };
  ...
  async cleanup(): Promise<void> {
    // Nothing to clean up in memory storage
  }
} 